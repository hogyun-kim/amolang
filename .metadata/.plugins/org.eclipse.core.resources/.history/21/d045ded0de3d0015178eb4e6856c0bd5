...
public class WebGeneratorController {
	...
	public String getComponentInfo(HttpServletRequest request,
		...
		if(component == null) {
			component = getcomponentservice.getComponent(uri_token);
		}
		...
		if(component.dbNecessity()) {
			...
			reAttributes.addFlashAttribute("dbschema", dbschema);
		}	
		...
		if(!parammap.isEmpty()) {
			...
			while(token.hasMoreTokens() && iterator.hasNext()) {
				reAttributes.addFlashAttribute(token.nextToken(), parammap.get(iterator.next()));
			}
